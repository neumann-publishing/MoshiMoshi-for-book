...
const ParticipantVideo = memo(({ participant, speakerState }: ParticipantVideoProps) => {
	const videoRef = useRef<HTMLVideoElement>(null);
	const audioRef = useRef<HTMLAudioElement>(null);
	const [videoElementReady, setVideoElementReady] = useState(false);
	const [audioElementReady, setAudioElementReady] = useState(false);
	const retryCountRef = useRef<Record<string, number>>({ video: 0, audio: 0 });
			...
			try {
				...
				const playVideo = async () => {
					...
					try {
						...
						console.log(`Successfully playing video for ${participant.userName}`);
						...
						if (error instanceof DOMException && error.name === 'AbortError') {
							...
							if (retryCountRef.current.video < 5) {
								...
								console.warn(`Video play aborted for ${participant.userName}, retrying... (${retryCountRef.current.video}/5)`);
								await new Promise(resolve => setTimeout(resolve, 1000));
								if (mounted) {
									playVideo();
								}
							} else {
								console.error(`Max retries reached for ${participant.userName} video playback`);
							}
						} else {
							console.error(`Failed to play video for ${participant.userName}:`, error);
						}
					}
				};
				...
				console.error(`Error setting up video for ${participant.userName}:`, error);
			}
		};
			...
			try {
				...
				const playAudio = async () => {
					...
					try {
						...
						console.log(`Successfully playing audio for ${participant.userName}`);
						retryCountRef.current.audio = 0; // リトライカウンターをリセット
					} catch (error) {
						// ユーザーインタラクションが必要な場合
						if (error instanceof DOMException && error.name === 'NotAllowedError') {
							console.warn(`Audio autoplay blocked for ${participant.userName}, will retry on user interaction`);
							// ユーザーインタラクション後に自動的に再生を試みる
							const resumePlayback = () => {
								if (audioRef.current && mounted) {
									audioRef.current.play().catch(e => console.warn("Error resuming audio on interaction:", e));
								}
							...
							if (retryCountRef.current.audio < 5) {
								...
								console.warn(`Audio play aborted for ${participant.userName}, retrying... (${retryCountRef.current.audio}/5)`);
								await new Promise(resolve => setTimeout(resolve, 1000));
								if (mounted) {
									playAudio();
								}
							} else {
								console.error(`Max retries reached for ${participant.userName} audio playback`);
							}
						} else {
							console.error(`Failed to play audio for ${participant.userName}:`, error);
						}
					}
				};
				...
				console.error(`Error setting up audio for ${participant.userName}:`, error);
			}
		};
	...
	}, [participant.audioStream, participant.userName, speakerState, audioElementReady]);
...
export function MeetingPanel({
	...
	// 参加者のストリーム情報を管理
	const [participantStreams, setParticipantStreams] = useState<Record<string, ParticipantStream>>({});
	...
	const closeMeeting = () => {
		...
		if (socketRef.current) {
			socketRef.current.emit("leaveMeeting", { meetingUuid: meeting.uuid }, (result) => {
				if (result.success) {
	...
	useEffect(() => {
		...
		const setupConsumerForParticipant = async (
			...
			if (!participant.producerId) {
				console.log(`Participant ${participant.userName} has no producer ID yet`);
				return;
			}
					...
					socket.emit(
						...
						async (response: ConsumeResponse) => {
							try {
								...
								setParticipantStreams(prev => {
									...
									if (response.kind === "video") {
										if (existingParticipant.videoStream) {
											existingParticipant.videoStream.getTracks().forEach(track => track.stop());
										}
										updatedParticipant = {
											...existingParticipant,
											userId: String(participant.userId),
										...
										if (existingParticipant.audioStream) {
											existingParticipant.audioStream.getTracks().forEach(track => track.stop());
										}
										updatedParticipant = {
											...existingParticipant,
											userId: String(participant.userId),
									...
									console.log(`Updated participant stream for ${participant.userName}, kind: ${response.kind}`);
								...
								console.log(`Received ${response.kind} stream from ${participant.userName}`);
								...
								console.error(`Error consuming ${response.kind} from ${participant.userName}:`, error);
							}
						},
					);
				},
		...
		socket.emit("getRouterRtpCapabilities", async (rtpCapabilities) => {
			...
			// 既存の参加者情報を取得
			socket.emit("getParticipants", { meetingUuid: meeting.uuid }, async (existingParticipants) => {
				...
				for (const existingParticipant of existingParticipants) {
					...
					if (existingParticipant.producerId) {
						console.log(`Setting up stream for existing participant: ${existingParticipant.userName}`);
						await setupConsumerForParticipant(existingParticipant, device, socket, speakerState);
					} else {
						console.log(`Existing participant ${existingParticipant.userName} has no producer ID yet`);
					}
				}
			});
			...
			socket.on("newProducer", async (participant: ParticipantInfo) => {
				...
				// 新しい参加者のストリームを設定
				await setupConsumerForParticipant(participant, device, socket, speakerState);
			});
